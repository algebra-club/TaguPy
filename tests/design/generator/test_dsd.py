...
def test_init_valid_input(correct_input):
    for a, exp in zip(correct_input, correct_input):
        init = DSD(a).n_rep
        assert init == exp,\
            f'self.n_rep expected {exp}, got {init}'
...
def test_get_exmatrix_invalid_input():
    # first 4 elements in arg don't meet the requirment of either n-factor and n_fake
    # last 4 elements in arg don't meet the requirement of sum_factor(=n_factor+n_fake)
    ...
    for a, c in zip(arg0[:5], cor[:5]):
        with pytest.raises(AssertionError) as e:
            model.get_exmatrix(n_factor=a, n_fake=c)
        assert f'{a}' in f'{e.value}',\
            f"NoReasons: Inform the AssertionError reasons, got {e.value}"
    for c, a in zip(cor[:5], arg1[:5]):
        with pytest.raises(AssertionError) as e:
            model.get_exmatrix(n_factor=c, n_fake=a)
        assert f'{a}' in f'{e.value}',\
            f"NoReasons: Inform the AssertionError reasons, got {e.value}"
...
def test_get_exmatrix_valid_output(correct_input):
    ...
    for fac in correct_input:
        ...
        if fac % 2 == 1:
            sum_fac += 1
        ex_mat = model.get_exmatrix(n_factor=fac, n_fake=n_fake)
