...
def test_valid_output(correct_input):
    for func, num in correct_input:
        for sum_fac in num:
            ...
            diag = np.diag(c_mat)
            assert (diag == 0).all(),\
                f'Error: c_mat expected a matrix with zero diagonal, got {diag}'
            # identity matrix
            if sum_fac == 22 or sum_fac == 34:
                assert (c_mat @ c_mat.T == (sum_fac - 2) * np.eye(sum_fac)).all(),\
                    f'Error: CC′ expected {sum_fac-2}*I_{sum_fac},{sum_fac}, got {c_mat @ c_mat.T}'
            else:
                assert (c_mat @ c_mat.T == (sum_fac - 1) * np.eye(sum_fac)).all(),\
                    f'Error: CC′ expected {sum_fac-1}*I_{sum_fac},{sum_fac}, got {c_mat @ c_mat.T}'
                # zero only diagonal
